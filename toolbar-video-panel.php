...
class bbp_5o1_video_panel {
	...
	function panel_entry($items) {
		...
		$random_video[] = "https://www.youtube.com/watch?v=RSJbYWPEaxw"; // Hallelujah (Bon Jovi)
		$random_video[] = "https://www.youtube.com/watch?v=XCspzg9-bAg"; // Batroll'd
		$random_video[] = "https://www.youtube.com/watch?v=RZ-uV72pQKI"; // Pure Imagination
		$random_video[] = "https://www.youtube.com/watch?v=rgUrqGFxV3Q";	// Lights Out
		$random_video[] = "https://www.vimeo.com/26753142"; // Share the Rainbow

		$video_provider['Dailymotion'] = "//www.dailymotion.com/";
		$video_provider['LiveLeak'] = "//www.liveleak.com/";
		$video_provider['Metacafe'] = "//www.metacafe.com/";
		//$video_provider['RedTube'] = "//www.redtube.com/";
		$video_provider['Vimeo'] = "//www.vimeo.com/";
		$video_provider['YouTube'] = "//www.youtube.com/";
...
HTML;
	...
	function embed_flash ( $video_code, $flash_vars = null, $atts = null ) {
		$dimensions = bbp_5o1_video_panel::getDimensions($atts);
		return '<embed src="' . $video_code. '" width="' . $dimensions['width'] . '" height="' . $dimensions['height'] . '" flashVars="' . $flash_vars. '"  wmode="transparent" allowFullScreen="true" allowScriptAccess="always" pluginspage="//www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash"></embed>';
	}
	...
	function youtube( $atts = null, $content = null ) {
		...
		if ( is_ssl() )
			return bbp_5o1_video_panel::embed_iframe( "//www.youtube.com/embed/${video_code['v']}", $atts );
	...
	function dailymotion( $atts = null, $content = null ) {
		...
		return bbp_5o1_video_panel::embed_iframe( "//www.dailymotion.com/embed${video_code[0]}", $atts );
	}
	...
	function vimeo( $atts = null, $content = null ) {
		...
		return bbp_5o1_video_panel::embed_iframe( "//player.vimeo.com/video${video_code}?portrait=0", $atts );
	}

	function metacafe( $atts = null, $content = null ) {
		$video_code = parse_url( $content, PHP_URL_PATH );
		$video_code = explode( '/', $video_code );
		return bbp_5o1_video_panel::embed_flash( "//www.metacafe.com/fplayer/${video_code[2]}/what_if.swf", "playerVars=showStats=yes|autoPlay=no", $atts );
	}
	...
	function liveleak( $atts = null, $content = null ) {
		...
		return bbp_5o1_video_panel::embed_flash( "//www.liveleak.com/e/${video_code['i']}", null, $atts );
	}

	function redtube( $atts = null, $content = null ) {
		$video_code = str_replace( '/', '', parse_url( $content, PHP_URL_PATH ) );
		return bbp_5o1_video_panel::embed_flash( "//embed.redtube.com/player/?style=redtube&id=${video_code}", "autostart=false", $atts );
	}
}
